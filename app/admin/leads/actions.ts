...
export async function createLeadAction(formData: FormData) {
  ...
  if (memo) await supabase.from("lead_notes").insert({ lead_id: data.id, author_id: profile.id, content: memo });
  revalidatePath("/admin/leads"); revalidatePath("/admin/dashboard"); revalidatePath("/admin/contracts");
...
export async function updateLeadAction(formData: FormData) {
  ...
  const payload: any = {
    name: cleanText(formData.get("name")), phone: cleanText(formData.get("phone")),
    contact_method: cleanText(formData.get("contact_method")) || null,
    insurance_company: cleanText(formData.get("insurance_company")) || null,
    status: cleanText(formData.get("status")) || "NEW",
    manager_name: cleanText(formData.get("manager_name")) || null
  };
  if (profile.role === "ADMIN") payload.assigned_to = cleanText(formData.get("assigned_to")) || null;
  const supabase = createSupabaseServerClient();
  const { error } = await supabase.from("leads").update(payload).eq("id", id).is("deleted_at", null);
  if (error) return { ok: false, error: error.message };
  revalidatePath("/admin/leads"); revalidatePath(`/admin/leads/${id}`); revalidatePath("/admin/contracts");
  return { ok: true };
}
...
export async function upsertLeadNoteAction(formData: FormData) {
  ...
  if (!leadId || !content) return { ok: false, error: "메모 내용을 입력해 주세요." };
  const supabase = createSupabaseServerClient();
  const noteId = cleanText(formData.get("note_id"));
  const result = noteId
    ? await supabase.from("lead_notes").update({ content, updated_at: new Date().toISOString() }).eq("id", noteId)
    : await supabase.from("lead_notes").insert({ lead_id: leadId, author_id: profile.id, content });
  if (result.error) return { ok: false, error: result.error.message };
  revalidatePath("/admin/leads"); revalidatePath(`/admin/leads/${leadId}`);
  return { ok: true };
}
