  ...
  function submit(fd: FormData) {
    startTransition(async () => {
      ...
      if (r.ok) {
        ...
        setMsg(r.error ?? "저장 중 오류가 발생했습니다.");
      }
    });
  }

  function saveMemo(fd: FormData) {
    startTransition(async () => {
      const r = await upsertLeadNoteAction(fd);

      setMsg(
        r.ok
          ? "메모가 저장되었습니다."
          : r.error ?? "메모 저장 중 오류가 발생했습니다."
      );
    });
  }
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-900 text-slate-400">
            <tr>
              ...
              <th className="p-3 text-left">상대보험사</th>
          ...
          <tbody className="divide-y divide-slate-800">
            {rows.map((row) => (
              <Fragment key={row.id}>
                ...
                {memoId === row.id && (
                  <tr>
                    <td colSpan={9} className="bg-slate-950 p-4">
