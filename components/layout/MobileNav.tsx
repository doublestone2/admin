...
export function MobileNav({ profile }: { profile: Profile }) {
  ...
  const links = [
    ["/admin/dashboard", "대시보드"], ["/admin/leads", "교통사고 DB관리"], ["/admin/insurance", "보험사 DB"],
  ...
  return <div className="lg:hidden">
    ...
    {open && <div className="absolute left-4 right-4 top-16 z-50 rounded-xl border border-slate-700 bg-slate-950 p-3 shadow-xl">
      <div className="mb-3 text-sm text-slate-400">{profile.name} · {profile.role}</div>
      <div className="grid gap-1">{links.map(([href,label]) => <Link key={href} onClick={()=>setOpen(false)} className="rounded-lg px-3 py-2 text-sm font-bold text-slate-200 hover:bg-slate-800" href={href}>{label}</Link>)}</div>
    </div>}
  </div>;
}
